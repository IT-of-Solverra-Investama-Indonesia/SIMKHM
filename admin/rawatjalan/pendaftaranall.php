...
//   Pagination
    ...
    $result = $koneksi->query("SELECT COUNT(*) as count FROM registrasi_rawat WHERE status_antri != 'Belum Datang' AND DATE_FORMAT(jadwal, '%Y-%m-%d') >= '$tgl_mulaii'");
...
// End Pagination
  ...
  $pasien=$koneksi->query("SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat WHERE status_antri != 'Belum Datang' and DATE_FORMAT(jadwal, '%Y-%m-%d') >= '$tgl_mulai'  ORDER BY jadwal DESC LIMIT $start, $limit;"); 
?>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="width:200px">
                                <a href="index.php?halaman=pendaftaranall&syncCek=<?= $data['idrawat']?>&jadwal=<?= $data['jadwal']?>&pasien=<?= $data['nama_pasien']?>"  onclick="return confirm('Apakah Anda yakin ingin menyingkronkan data pasien yang di pilih ???')" class="btn btn-sm btn-primary" style="display: none;"><i class="bi bi-arrow-repeat"></i> Cek Sync</a>
                                <?php if($data['status_sinc'] != 'Berhasil'){?>
                                    <a href="index.php?halaman=pendaftaranall&sync=<?= $data['idrawat']?>&jadwal=<?= $data['jadwal']?>&pasien=<?= $data['nama_pasien']?>&page=<?= $cekPage?>"  onclick="return confirm('Apakah Anda yakin ingin menyingkronkan data pasien yang di pilih ???')" class="btn btn-sm btn-warning"><i class="bi bi-arrow-repeat"></i>Sync</a>
        ...
        if ($result->num_rows > 0) {
            ...
            if(!isset($_GET['page'])){
                ...
                echo "
                    <script>
                        alert('Berhasil Menyingkronkan Pasien, Silahkan Cek Kembali Apakah Ada Data Yang Gagal Terkirim !');
                        document.location.href='index.php?halaman=pendaftaranall&page=$_GET[page]';
                    </script>
                ";
            }
        } else {
