<?php if(!isset($_GET['pasienInap'])){?>
  <?php
      if(isset($_GET['detail']) and isset($_GET['tgl'])){
        ...
        $pasien=$koneksi->query("SELECT *, registrasi_rawat.kasir, registrasi_rawat.shift FROM registrasi_rawat WHERE perawatan = 'Rawat Jalan' and (status_antri = 'Datang' or status_antri = 'Pembayaran') and date_format(jadwal, '%Y-%m-%d') = '$date' GROUP BY registrasi_rawat.idrawat;"); 
      }else{
        $date= date("Y-m-d");
        $shift= $_SESSION["shift"];
        // var_dump($shift);
        ...
        $pasien=$koneksi->query("SELECT *, registrasi_rawat.kasir, registrasi_rawat.shift FROM registrasi_rawat WHERE registrasi_rawat.shift = '$shift' and perawatan = 'Rawat Jalan' and (status_antri = 'Datang' or status_antri = 'Pembayaran') and date_format(jadwal, '%Y-%m-%d') = '$date' GROUP BY registrasi_rawat.idrawat;"); 
      }
  ?>
  <!DOCTYPE html>
  <html lang="en">
   ...
   <body>
     <main>
      <div class="">
        ...
        <nav>
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="index.php?halaman=daftarbayar" style="color:blue;">Daftar Pembayaran</a></li>
          </ol>
        </nav>
      </div><!-- End Page Title -->
        <section class="">
          <div class="">
            <div class="row">
              ...
              <?php 
                if(isset($_POST['search'])){
                  echo "
                    <script>
                      document.location.href='index.php?halaman=daftarbayar&detail&tgl=$_POST[tgl]';
                    </script>
                  ";
                }
              ?>
            ...
            <?php foreach ($pasien as $pecah) : ?>
              ...
              $biayaRawat = $koneksi->query("SELECT * FROM biaya_rawat WHERE idregis= '$pecah[idrawat]' ORDER BY id DESC LIMIT 1")->fetch_assoc();
               if(isset($biayaRawat["poli"])){$poli=$biayaRawat["poli"];}else{$poli=0;};
               if(isset($biayaRawat["periksa_lab"])){$periksa_lab=$biayaRawat["periksa_lab"];}else{$periksa_lab=0;};
               if(isset($biayaRawat["biaya_lab"])){$biaya_lab=$biayaRawat["biaya_lab"];}else{$biaya_lab=0;};
               if(isset($biayaRawat["biaya_lain"])){$biaya_lain=$biayaRawat["biaya_lain"];}else{$biaya_lain=0;};
               if(isset($biayaRawat["total_lain"])){$total_lain=$biayaRawat["total_lain"];}else{$total_lain=0;};
               if(isset($biayaRawat["potongan"])){$potongan=$biayaRawat["potongan"];}else{$potongan=0;};
              ?>
            <tr>
                <td><?php echo $no; ?></td>
