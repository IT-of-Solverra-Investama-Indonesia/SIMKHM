...
$single = $koneksimaster->query("SELECT * FROM gajidokter WHERE tgl = '" . $date . "' AND shiftgaji = '" . (htmlspecialchars($_GET['shiftgaji']) ?? $_SESSION['shift']) . "' AND unit = '" . $unit . "' ORDER BY idgaji DESC LIMIT 1")->fetch_assoc();
?>
...
<div style="max-width: 58mm; padding: 2mm;">
    ...
    <?php
    $total = 0;
    $getData = $koneksimaster->query("SELECT * FROM gajidokter WHERE tgl = '" . $date . "' AND shiftgaji = '" . (htmlspecialchars($_GET['shiftgaji']) ?? $_SESSION['shift']) . "' AND unit = '" . $unit . "' ORDER BY idgaji DESC");
