    ...
    <?php if ($_SESSION['admin']['level'] == 'dokter') { ?>
        <div>
            <button type="button" onclick="changeJenis('Injeksi')" class="btn btn-primary btn-sm text-right"
                data-bs-toggle="modal" data-bs-target="#exampleModal45"><?= $_SESSION['admin']['level'] == 'dokter' ? 'Request' : 'Add' ?> Injeksi Jadi</button>
            <button type="button" onclick="changeJenis('Oral')" class="btn btn-primary btn-sm text-right"
                data-bs-toggle="modal" data-bs-target="#exampleModal45"><?= $_SESSION['admin']['level'] == 'dokter' ? 'Request' : 'Add' ?> Oral Jadi</button>
            <button type="button" onclick="changeJenis('Oral')" class="btn btn-primary btn-sm text-right"
                data-bs-toggle="modal" data-bs-target="#exampleModal2"><?= $_SESSION['admin']['level'] == 'dokter' ? 'Request' : 'Add' ?> Oral Racik</button>
        </div>
    <?php } ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($in['see_apotek_at'] == null) { ?>
                                ...
                                <?php if ($_SESSION['admin']['level'] != 'dokter') { ?>
                                    <a href="<?= $urlBase ?>&seeApotek=<?= $in['idobat'] ?>" class="btn btn-sm btn-success"><i class="bi bi-check"></i></a>
                                <?php } ?>
                            <?php } ?>
                        </td>
                    </tr>
                        ...
                        <div class="col-6">
                            <div class="table-responsive">
                                <table class="table table-hover table-striped" style="font-size: 12px;">
                                    ...
                                    <tbody>
                                        <?php
                                        $injek = $koneksi->query("SELECT * FROM obat_rm_request WHERE idrm = '$_GET[id]' AND tgl_pasien='$_GET[tgl]' AND see_apotek_at IS NULL");
                                        $urlBase = "index.php?halaman=cttpenyakit_obat&id=" . htmlspecialchars($_GET['id']) . "&inap&tgl=" . htmlspecialchars($_GET['tgl']);
                                        $noo = 1;
                                        foreach ($injek as $in) {
                                        ?>
