...
<?php if (!isset($_GET['detail'])) { ?>
  <?php
  $fal = $koneksi->query("SELECT * FROM humpty_igd WHERE pasien = '$pecah[nama_pasien]' AND tipe='inap'");
  ?>
              ...
              <div class="col-md-12">
                ...
                <div class="card" style="margin-top:10px">
                  <div class="card-body col-md-12">
                    ...
                    <input type="hidden" class="form-control" id="inputName5" name="tgl_masuk" value="<?php echo $pecah['jadwal'] ?>">
                          ...
                          <td>
                            <div class="dropdown">
                              ...
                              <ul class="dropdown-menu">
                                ...
                                <li><a href="index.php?halaman=falanakinap&id=<?php echo $pecah["id"]; ?>&detail" class="dropdown-item" style="text-decoration: none; margin-left: 1px; font-weight: bold;"><i class="bi bi-eye-fill" style="color:darkblue;"></i> Detail</a></li>
...
if (isset($_POST['save'])) {
  $ob = $_POST["obat"];
  $obat = implode(", ", $ob);
  $jenis_ket = implode(", ", $_POST['jenis_ket']);

  $no1 = isset($_POST['no1']) ? 'on' : 'off';
  $no2 = isset($_POST['no2']) ? 'on' : 'off';
  $no3 = isset($_POST['no3']) ? 'on' : 'off';
  $no4 = isset($_POST['no4']) ? 'on' : 'off';
  $no5 = isset($_POST['no5']) ? 'on' : 'off';
  $no6 = isset($_POST['no6']) ? 'on' : 'off';
  $no7 = isset($_POST['no7']) ? 'on' : 'off';
  $no8 = isset($_POST['no8']) ? 'on' : 'off';

  $koneksi->query("INSERT INTO humpty_igd (tgl, jenis_ket, usia, jk, diagnosis, kognitif, lingkungan, respon, medik, total_skor, jenis_skor, perawat, obat, no1, no2, no3, no4, no5, no6, no7, no8, pasien, no_rm, tgl_masuk, tipe) VALUES ('$_POST[tgl]', '$jenis_ket', '$_POST[usia]', '$_POST[jk]', '$_POST[diagnosis]', '$_POST[kognitif]', '$_POST[lingkungan]', '$_POST[respon]', '$_POST[medik]', '$_POST[total_skor]', '$_POST[jenis_skor]', '$perawat', '" . $obat . "', '$no1','$no2','$no3','$no4','$no5','$no6','$no7','$no8','$_POST[pasien]','$_POST[no_rm]', '$_POST[tgl_masuk]', 'inap')");

  echo "
    <script>
      alert('Data berhasil ditambah');
      document.location.href='index.php?halaman=falanakinap&id=$_GET[id]';
    </script>
  ";
}
?>