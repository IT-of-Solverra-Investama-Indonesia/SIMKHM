<div class="">
    ...
    if (isset($_POST['simpan_catatan'])) {
        ...
        $query = "INSERT INTO catatan_farmasi (idrawat_registrasi, nama_pasien, jadwal, no_rm, catatan, created_by) 
                  VALUES ('$idrawat', '$nama_pasien', '$jadwal', '$no_rm', '$catatan', '$created_by')";

        if ($koneksi->query($query)) {
            echo "<script>alert('Catatan farmasi berhasil disimpan!');</script>";
            echo "<script>window.location.href='index.php?halaman=catatanfarmasi&idrawat=" . $idrawat . "';</script>";
        } else {
            echo "<script>alert('Gagal menyimpan catatan farmasi!');</script>";
        }
    }

    // UPDATE - Update Catatan Farmasi
    if (isset($_POST['update_catatan'])) {
        ...
        $query = "UPDATE catatan_farmasi SET catatan = '$catatan' WHERE id = '$id'";

        if ($koneksi->query($query)) {
            echo "<script>alert('Catatan farmasi berhasil diupdate!');</script>";
            echo "<script>window.location.href='index.php?halaman=catatanfarmasi&idrawat=" . htmlspecialchars($_GET['idrawat']) . "';</script>";
        } else {
            echo "<script>alert('Gagal mengupdate catatan farmasi!');</script>";
        }
    }

    // DELETE - Hapus Catatan Farmasi
    if (isset($_GET['hapus_catatan'])) {
        $id = htmlspecialchars($_GET['hapus_catatan']);
        $query = "DELETE FROM catatan_farmasi WHERE id = '$id'";

        if ($koneksi->query($query)) {
            echo "<script>alert('Catatan farmasi berhasil dihapus!');</script>";
            echo "<script>window.location.href='index.php?halaman=catatanfarmasi&idrawat=" . htmlspecialchars($_GET['idrawat']) . "';</script>";
        } else {
            echo "<script>alert('Gagal menghapus catatan farmasi!');</script>";
        }
    }

    // READ - Ambil Riwayat Catatan Farmasi
    $riwayat_catatan = $koneksi->query("SELECT * FROM catatan_farmasi WHERE idrawat_registrasi = '" . htmlspecialchars($_GET['idrawat']) . "' ORDER BY created_at DESC");
    ?>
    <div class="card shadow p-2 mb-2">
        ...
        <form method="post">
            <label for=""><b><i class="bi bi-journal-text"></i> Input Catatan Farmasi :</b></label>
            <textarea name="catatan" class="form-control mb-2" id="" rows="4" required placeholder="Masukkan catatan farmasi..."></textarea>
    ...
    <div class="card shadow p-2 mt-0">
        <h5 class="card-title">Riwayat Catatan Farmasi</h5>
                    ...
                    ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="index.php?halaman=catatanfarmasi&idrawat=<?= htmlspecialchars($_GET['idrawat']) ?>&hapus_catatan=<?= $row['id'] ?>"
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header bg-info text-white">
                                            <h5 class="modal-title" id="viewModalLabel">
                                                <i class="bi bi-journal-text"></i> Detail Catatan Farmasi
                                            </h5>
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <hr>
                                            <div class="mb-3">
                                                <h6 class="text-muted mb-2"><i class="bi bi-journal-medical"></i> Catatan Farmasi:</h6>
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header bg-warning">
                                            <h5 class="modal-title" id="editModalLabel">
                                                <i class="bi bi-pencil-square"></i> Edit Catatan Farmasi
                                            </h5>
                                        ...
                                        <form method="post">
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    <label class="form-label"><b><i class="bi bi-journal-text"></i> Catatan Farmasi:</b></label>
                                                    <textarea name="catatan_edit" class="form-control" rows="6" required placeholder="Masukkan catatan farmasi..."><?= htmlspecialchars($row['catatan']) ?></textarea>
                                                </div>
