...
if (isset($_GET['inap']) and !isset($_GET['detail'])) {
  ...
  $pasien = $koneksi->query("SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat WHERE  (status_antri!='Pulang') and perawatan ='Rawat Inap' ORDER BY jadwal DESC, DATE_FORMAT(jadwal, '%Y-%m-%d') DESC, FIELD(LEFT(antrian, 1), 'M', 'S', 'P'), CAST(SUBSTRING(antrian, 2) AS UNSIGNED) DESC, idrawat DESC");
} elseif (isset($_GET['racik'])) {
  $date_start = date('Y-m-d', strtotime('-1 days'));
  $date_end = date('Y-m-d');
  if (isset($_GET['pasrajal'])) {
    $queryPasien = "SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat INNER JOIN biaya_rawat ON biaya_rawat.idregis = registrasi_rawat.idrawat WHERE (apoteker_check_at IS NULL OR apoteker_check_at = '' OR apoteker_check_at = '0000-00-00 00:00:00') AND DATE_FORMAT(jadwal, '%Y-%m-%d') BETWEEN '$date_start' AND '$date_end' AND (status_antri='Datang' or  status_antri='Pembayaran' or status_antri='Selesai' or status_antri='Pulang') and perawatan ='Rawat Jalan'" . $queryKey . " ORDER BY jadwal DESC, DATE_FORMAT(jadwal, '%Y-%m-%d') DESC, FIELD(LEFT(antrian, 1), 'M', 'S', 'P'), CAST(SUBSTRING(antrian, 2) AS UNSIGNED) DESC, idrawat DESC";
  } elseif (isset($_GET['pasinap'])) {
    $queryPasien = "SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat INNER JOIN biaya_rawat ON biaya_rawat.idregis = registrasi_rawat.idrawat WHERE (apoteker_check_at IS NULL OR apoteker_check_at = '' OR apoteker_check_at = '0000-00-00 00:00:00') AND DATE_FORMAT(jadwal, '%Y-%m-%d') BETWEEN '$date_start' AND '$date_end' AND (status_antri='Datang' or  status_antri='Pembayaran' or status_antri='Selesai' or status_antri='Pulang') and perawatan ='Rawat Inap'" . $queryKey . " ORDER BY jadwal DESC, DATE_FORMAT(jadwal, '%Y-%m-%d') DESC, FIELD(LEFT(antrian, 1), 'M', 'S', 'P'), CAST(SUBSTRING(antrian, 2) AS UNSIGNED) DESC, idrawat DESC";
  } else {
    $queryPasien = "SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat INNER JOIN biaya_rawat ON biaya_rawat.idregis = registrasi_rawat.idrawat WHERE (apoteker_check_at IS NULL OR apoteker_check_at = '' OR apoteker_check_at = '0000-00-00 00:00:00') AND DATE_FORMAT(jadwal, '%Y-%m-%d') BETWEEN '$date_start' AND '$date_end' AND (status_antri!='Belum Datang') " . $queryKey . " ORDER BY jadwal DESC, DATE_FORMAT(jadwal, '%Y-%m-%d') DESC, FIELD(LEFT(antrian, 1), 'M', 'S', 'P'), CAST(SUBSTRING(antrian, 2) AS UNSIGNED) DESC, idrawat DESC";
  }
  ...
  if (isset($_GET['inap'])) {
    $pasien = $koneksi->query("SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat WHERE  (status_antri='Datang' or  status_antri='Pembayaran' or status_antri='Selesai' or status_antri='Pulang') and perawatan ='Rawat Inap' and REPLACE(REPLACE(REPLACE(no_rm, '\t', ' '), '  ', ' '), '  ', ' ')  = '$_GET[detail]'  ORDER BY jadwal DESC, DATE_FORMAT(jadwal, '%Y-%m-%d') DESC, FIELD(LEFT(antrian, 1), 'M', 'S', 'P'), CAST(SUBSTRING(antrian, 2) AS UNSIGNED) DESC, idrawat DESC limit 1;");
  } else {
    $pasien = $koneksi->query("SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat WHERE (status_antri='Datang' or status_antri='0' or status_antri='Pembayaran' or status_antri='Selesai') and REPLACE(REPLACE(REPLACE(no_rm, '\t', ' '), '  ', ' '), '  ', ' ')  = '$_GET[detail]' and perawatan ='Rawat Jalan' ORDER BY jadwal DESC, DATE_FORMAT(jadwal, '%Y-%m-%d') DESC, FIELD(LEFT(antrian, 1), 'M', 'S', 'P'), CAST(SUBSTRING(antrian, 2) AS UNSIGNED) DESC, idrawat DESC Limit 1;");
  }
  ...
  // if(isset($_GET['rajal']))
  $queryPasien = "SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat WHERE (status_antri!='') " . $queryKey . " ORDER BY jadwal DESC, DATE_FORMAT(jadwal, '%Y-%m-%d') DESC, FIELD(LEFT(antrian, 1), 'M', 'S', 'P'), CAST(SUBSTRING(antrian, 2) AS UNSIGNED) DESC, idrawat DESC";
  ...
  $pasien = $koneksi->query("SELECT *, DATE_FORMAT(jadwal, '%Y-%m-%d') as tgl FROM registrasi_rawat WHERE (status_antri='Datang' or status_antri='0') and perawatan ='Rawat Jalan' AND DATE_FORMAT(jadwal, '%Y-%m-%d') = '$date' " . $caraBayarFilter . " ORDER BY jadwal DESC, DATE_FORMAT(jadwal, '%Y-%m-%d') DESC, FIELD(LEFT(antrian, 1), 'M', 'S', 'P'), CAST(SUBSTRING(antrian, 2) AS UNSIGNED) DESC, idrawat DESC;");
}
                                        ...
                                        <?php } else { ?>
                                          ...
                                          <ul class="dropdown-menu">
                                            ...
                                            <?php } else { ?>
                                              ...
                                              <li><a href="index.php?halaman=riwayatLab&id=<?php echo $pecah["idrawat"]; ?>&rm=<?php echo $pecah["no_rm"] ?>&inap&tgl=<?php echo $pecah["tgl"]; ?>" class="dropdown-item" style="text-decoration: none; margin-left: 1px; font-weight: bold;"><i class="bi bi-clipboard2-pulse" style="color: pink;"></i> Riwayat Lab</a></li>
                                            <?php } ?>
                                            <?php if (isset($_GET['inap'])) { ?>
                    ...
                    <script>
                      function callAntrian(id) {
                        if (confirm('Apakah Anda yakin ingin memanggil antrian?')) {
                          fetch('../api/api_getAntrianTerakhir.php?fungsi=addAntreanDokterToDB', {
                              method: 'POST',
                              headers: {
