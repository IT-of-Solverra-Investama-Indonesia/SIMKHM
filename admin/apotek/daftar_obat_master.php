...
<div class="card shadow-sm p-2">
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-warning" data-bs-target="#setMargin" onclick="upMargin('<?= $data['id']?>', '<?= $data['margin']?>')" data-bs-toggle="modal">
                                <?= $data['margin']?>%
                            </button>
                        </td>
    ...
    <?php 
        if(isset($_POST['setting'])){
            $koneksimaster->query("UPDATE master_obat SET margin = '".htmlspecialchars($_POST['margin'])."' WHERE id = '".htmlspecialchars($_POST['id2'])."'");
        }
    ?>
    <script>
