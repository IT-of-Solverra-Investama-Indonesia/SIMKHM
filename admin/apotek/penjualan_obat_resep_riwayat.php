<div class="">
        ...
        <div class="">
            ...
            <div class="card shadow p-2 mb-1">
                <form method="GET">
                    <div class="row">
                        <div class="col-5">
                            <input type="text" name="halaman" id="" value="penjualan_obat_resep_riwayat" class="form-control form-control-sm" placeholder="Cari Nota" hidden>
                    ...
                    <table class="table table-hover table-striped table-sm" style="font-size: 12px;">
                        ...
                        <tbody>
                            ...
                            if (isset($_GET['filter'])) {
                                ...
                                $getDataPenjualan = $koneksi->query("SELECT *, SUM((harga_umum - diskon_obat)*jumlah) as total FROM penjualan_resep WHERE tgl_jual BETWEEN '$dateStart' AND '$dateEnd' GROUP BY nota ORDER BY tgl_jual DESC");
                            } else {
                                $getDataPenjualan = $koneksi->query("SELECT *, SUM((harga_umum - diskon_obat)*jumlah) as total FROM penjualan_resep WHERE 1 = 1 GROUP BY nota ORDER BY tgl_jual DESC");
                            }
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="index.php?halaman=penjualan_obat_resep_riwayat&detail&nota=<?= $dataPenjualan['nota'] ?>" class="btn btn-sm btn-warning">
                                            <?= $dataPenjualan['nota'] ?>
                                        </a>
                                    </td>
    ...
    <?php } else { ?>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover table-sm" style="font-size: 12px;">
                        ...
                        <tbody>
                            <?php
                            $total = 0;
                            $getDetailPenjualan = $koneksi->query("SELECT * FROM penjualan_resep WHERE nota = '" . htmlspecialchars($_GET['nota']) . "' ORDER BY id_penjualan DESC");
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($_SESSION['admin']['level'] == 'sup') { ?>
                                            <a onclick="return confirm('Are you sure to delete this data ?')" href="index.php?halaman=penjualan_obat_resep_riwayat&detail&nota=<?= $dataDetail['nota'] ?>&delete=<?= $dataDetail['id_penjualan'] ?>" class="btn btn-sm btn-danger"><i class="bi bi-trash"></i></a>
                                        <?php } ?>
                                    </td>
                                </tr>
        ...
        if (isset($_GET['delete'])) {
            ...
            $deleteQuery = $koneksi->query("DELETE FROM penjualan_resep WHERE id_penjualan = '$idToDelete'");
            if ($deleteQuery) {
                echo "<script>alert('Data berhasil dihapus'); document.location.href='index.php?halaman=penjualan_obat_resep_riwayat&detail&nota=" . htmlspecialchars($_GET['nota']) . "';</script>";
            } else {
