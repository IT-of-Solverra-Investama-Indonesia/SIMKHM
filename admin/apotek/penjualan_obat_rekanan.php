<div class="">
    ...
    <h5 class="card-title">Penjualan Obat Rekanan</h5>
    ...
    <div class="card shadow p-2 mb-0">
        <form method="post">
            <div class="row">
                ...
                <div class="col-md-2">
                    <button class="btn btn-sm btn-secondary" name="addToCart">[+]</button>
                </div>
            </div>
        </form>
    </div>
    ...
    if (!isset($_SESSION['keranjang_obat_rekanan'])) {
        $_SESSION['keranjang_obat_rekanan'] = [];
    }
    ...
    if (isset($_POST['addToCart'])) {
        ...
        $harga = $obatLokalSingle['harga_beli'] * (110 / 100);
        $sub = $harga * $jumlah;
    ...
    if (isset($_GET['deleteCart'])) {
        ...
        if (isset($_SESSION['keranjang_obat_rekanan'][$index])) {
            unset($_SESSION['keranjang_obat_rekanan'][$index]);
            // Re-index array
            $_SESSION['keranjang_obat_rekanan'] = array_values($_SESSION['keranjang_obat_rekanan']);
        }
        // header('Location: ' . $_SERVER['PHP_SELF']);
        echo "
        <script>
            window.location.href = 'index.php?halaman=penjualan_obat_rekanan';
        </script>
        ";
        // exit;
    }
    ...
    <div class="card shadow mt-1 mb-1 p-2">
        <div class="table-responsive">
            <table class="table table-hover table-striped table-sm" style="font-size: 12px;">
                ...
                <tbody>
                    <?php if (!empty($_SESSION['keranjang_obat_rekanan'])) { ?>
                        <?php foreach ($_SESSION['keranjang_obat_rekanan'] as $index => $item) { ?>
                            <tr>
                                <td><?= $item['kode'] ?></td>
                                <td><?= $item['nama'] ?></td>
                                ...
                                <td>
                                    <a href="index.php?halaman=penjualan_obat_rekanan&deleteCart=<?= $index ?>" class="btn btn-sm btn-danger"
    ...
    <div class="card shadow p-2">
        <form method="POST">
            <div class="row">
                ...
                <div class="col-md-2">
                    <button class="btn btn-sm btn-secondary" name="saveTransaction" onclick="return confirm('Are You Sure To Save Transaction?')">Transaksi</button>
                </div>
            </div>
        </form>
    </div>
    ...
    if (isset($_POST['saveTransaction'])) {
        ...
        unset($_SESSION['keranjang_obat_rekanan']);
        echo "
        <script>
            alert('Transaksi Berhasil Disimpan!');
            window.location.href = '../apotek/penjualan_obat_rekanan_nota.php?nota=" . htmlspecialchars($_POST['nota']) . "';
        </script>
        ";
    }
    ?>
    <div class="card shadow p-2">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover table-sm" style="font-size: 12px;">
                ...
                <tbody>
                    <?php
                    $getRiwayat5 = $koneksi->query("SELECT *, SUM((harga_umum - diskon_obat)*jumlah) as hargaTotal FROM penjualan_rekanan GROUP BY nota ORDER BY id_penjualan DESC LIMIT 5");
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                <a target="_blank" href="../apotek/penjualan_obat_rekanan_nota.php?nota=<?= $data5['nota'] ?>" class="btn btn-sm btn-warning"><i class="bi bi-printer"></i></a>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
