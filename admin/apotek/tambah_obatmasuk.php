...
<div class="card shadow-sm p-2">
    <form method="POST">
        <div class="row">
            ...
            <div class="col-md-4">
                <label for="" class="mb-0 mt-2">Expired</label>
                <input type="date" required name="tgl_expired" id="" class="form-control mb-2">
            </div>
            <div class="col-md-4">
...
if (isset($_POST['save'])) {
    ...
    $koneksi->query("INSERT INTO pembelian_obat (jml_obat_minim, nama_obat, id_obat, jml_obat, produsen, bentuk, tipe, harga_beli, margininap, tgl_beli, tgl_expired, margin_jual, tipe_faktur, diskon, ppn, total) VALUES ('$jml_obat_minim', '$nama_obat', '$id_obat', '$jml_obat', '$produsen', '$bentuk', '$tipe', '$harga_beli', '$margininap', '$tgl_beli', '$tgl_expired', '$margin_jual', '$tipe_faktur', '$diskon', '$ppn', '$total') ");
...
<!-- 
ALTER TABLE `pembelian_obat` ADD `tgl_expired` DATE NULL DEFAULT NULL AFTER `total`;
ALTER TABLE `pembelian_obat` ADD `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `tgl_expired`;
ALTER TABLE `apotek` CHANGE `frekuensi` `tgl_expired` DATE NULL DEFAULT NULL; 
-->