        ...
        <div class="row g-1">
            ...
            <div class="col-3">
                ...
                <select name="jenis" id="" class="form-control form-control-sm">
                    <option value="Poli" <?= ($_GET['jenis'] ?? '') == 'Poli' ? 'selected' : '' ?>>Poli All</option>
                    <option value="PoliUmum" <?= ($_GET['jenis'] ?? '') == 'PoliUmum' ? 'selected' : '' ?>>PoliUmum</option>
                    <option value="PoliBPJS" <?= ($_GET['jenis'] ?? '') == 'PoliBPJS' ? 'selected' : '' ?>>PoliBPJS</option>
                    <option value="PoliAnak" <?= ($_GET['jenis'] ?? '') == 'PoliAnak' ? 'selected' : '' ?>>PoliAnak</option>
                    <option value="PoliPenyakitDalam" <?= ($_GET['jenis'] ?? '') == 'PoliPenyakitDalam' ? 'selected' : '' ?>>PoliPenyakitDalam</option>
                    <option value="PoliGigiUmum" <?= ($_GET['jenis'] ?? '') == 'PoliGigiUmum' ? 'selected' : '' ?>>PoliGigiUmum</option>
                    <option value="PoliGigiBPJS" <?= ($_GET['jenis'] ?? '') == 'PoliGigiBPJS' ? 'selected' : '' ?>>PoliGigiBPJS</option>
                    <option value="Inap" <?= ($_GET['jenis'] ?? '') == 'Inap' ? 'selected' : '' ?>>Inap</option>
                    <option value="Umum" <?= ($_GET['jenis'] ?? '') == 'Umum' ? 'selected' : '' ?>>Umum</option>
                    <option value="Resep" <?= ($_GET['jenis'] ?? '') == 'Resep' ? 'selected' : '' ?>>Resep</option>
                    <option value="Rekanan" <?= ($_GET['jenis'] ?? '') == 'Rekanan' ? 'selected' : '' ?>>Rekanan</option>
                    <option value="Internal" <?= ($_GET['jenis'] ?? '') == 'Internal' ? 'selected' : '' ?>>Internal</option>
                </select>
            </div>
            <div class="col-3">
                <br>
            ...
            <tbody>
                ...
                <?php if(isset($_GET['jenis']) && $_GET['jenis'] != 'Inap') { ?>
                    ...
                    if (isset($_GET['jenis']) && $_GET['jenis'] == 'Inap') {
                        ...
                        // $getObat = $koneksi->query("SELECT o.nama_obat, o.kode_obat, SUM(o.jml_dokter) AS jumlah_keluar FROM obat_rm AS o WHERE o.obat_igd != '' AND tgl_pasien >= '$_GET[mulai]' AND tgl_pasien <= '$_GET[hingga]' ORDER BY o.nama_obat DESC");
                    }
                    ...
                    if (isset($_GET['jenis']) && in_array($_GET['jenis'], ['PoliUmum', 'PoliBPJS', 'PoliAnak', 'PoliPenyakitDalam', 'PoliGigiUmum', 'PoliGigiBPJS'])) {
                        $queryKey = '';
                        ...
                        }elseif ($_GET['jenis'] == 'PoliGigiUmum') {
                            $queryKey .= " AND registrasi_rawat.carabayar = 'gigi umum'";
                        } elseif ($_GET['jenis'] == 'PoliGigiBPJS') {
                            $queryKey .= " AND registrasi_rawat.carabayar = 'gigi bpjs'";
                        }
                    ...
                    <?php foreach ($getObat as $obat) { ?>
                        <?php
                        if ($_GET['jenis'] == 'Inap' or $_GET['jenis'] == 'Poli' or in_array($_GET['jenis'], ['PoliUmum', 'PoliBPJS', 'PoliAnak', 'PoliPenyakitDalam' ,'PoliGigiUmum', 'PoliGigiBPJS'])) {
                        ...
                        <tr>
                            ...
                            <td>Rp <?= number_format($hargaJual == '' ? $hargaJual = 0 : $hargaJual, 0, 0, '.'); ?></td>
                            <td>Rp <?= number_format($hargaObatMasuk == '' ? $hargaObatMasuk = 0 : $hargaObatMasuk, 0, 0, '.'); ?></td>
                ...
                <?php if (isset($_GET['jenis']) && $_GET['jenis'] == 'Inap') { ?>
                    <?php
                    $getObat = $koneksi->query("SELECT *, SUM(jml_dokter) AS jumlah_keluar, (SELECT harga_beli FROM apotek WHERE tgl_beli <= DATE_FORMAT(o.created_at, '%Y-%m-%d') AND id_obat = o.kode_obat ORDER BY tgl_beli DESC LIMIT 1) AS harga_beli, (SELECT margininap FROM apotek WHERE tgl_beli <= DATE_FORMAT(o.created_at, '%Y-%m-%d') AND id_obat = o.kode_obat ORDER BY tgl_beli DESC LIMIT 1) AS margininap FROM obat_rm o WHERE tgl_pasien >= '$_GET[mulai]' AND tgl_pasien <= '$_GET[hingga]' AND obat_igd != '' GROUP BY o.kode_obat, harga_beli, margininap ORDER BY nama_obat DESC");
                    ...
                    <tr>
                        ...
                        <td>
                            Rp <?= number_format($harga = $obat['harga_beli'] * ($obat['margininap'] / 100), 0, 0, '.') ?>
                        </td>
                        <td>
                            Rp <?= number_format($obat['harga_beli'], 0, 0, '.') ?>
                        </td>
                        <td>
                            Rp <?= number_format($hargaTotal = $harga * $obat['jumlah_keluar'], 0, 0, '.') ?>
                        </td>
                        <td>
                            Rp <?= number_format($hargaTotalHPP = $obat['harga_beli'] * $obat['jumlah_keluar'], 0, 0, '.') ?>
                        </td>
                        <td>
                            Rp <?= number_format($laba = $hargaTotal - $hargaTotalHPP, 0, 0, '.') ?>
                        </td>
                        <td>
                            Rp <?= number_format($laba / $obat['jumlah_keluar'], 0, 0, '.') ?>
                        </td>
                    </tr>
                    <?php
                    $totalTotal += $hargaTotal;
